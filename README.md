...
*(Ajoutez ici 2-3 captures d'écran de l'application : page d'accueil, liste des véhicules, formulaire de réparation.)*
...
L'accès à l'application (à l'exception de la page de connexion) est réservé à l'administrateur, via un token API **Laravel Sanctum**.

Le compte administrateur est créé/synchronisé automatiquement par `DatabaseSeeder` à partir de deux variables d'environnement (voir `.env.example`) :
...
> ⚠️ Ne committez jamais vos vraies valeurs - `.env` est ignoré par Git. En production (Render), ces variables se définissent dans l'onglet **Environment** du service.
